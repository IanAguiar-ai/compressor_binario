...
##    with open("/home/user/Downloads/rascunho.txt", "r") as arq:
##        SEU_TEXTO:str = arq.read()

##    with open("/home/user/Documents/estudos/10_semestre/consultoria_2/apresentacao/o_que_apresentar.txt", "r") as arq:
##        SEU_TEXTO:str = arq.read()
...
##"""
##    SEU_TEXTO = SEU_TEXTO[:2000]
    
    #SEU_TEXTO:str = "abcdefghij " + "palavra "*100


    SEU_TEXTO = to_latin1(SEU_TEXTO)
    ...
    try:
        print(f"Original: {len(SEU_TEXTO)} bytes\nComprimido: {len(comprimido)}  + {len(dicionario_bits) + len(str(dicionario_bits['##']))} bytes = {(len(comprimido) + len(dicionario_bits) + len(str(dicionario_bits['##'])))} bytes")
        print(f"Ganho de {(1 - (len(comprimido) + len(dicionario_bits) + len(str(dicionario_bits['##'])))/len(SEU_TEXTO)) * 100:0.2f}%")
    except KeyError:
        print(f"Original: {len(SEU_TEXTO)} bytes\nComprimido: {len(comprimido)}  + {len(dicionario_bits)} bytes = {len(comprimido) + len(dicionario_bits)} bytes")
